...
import { ProviderNotFoundSignal, ProviderRequestError, resetThrottler } from "../../src/addresses/utils";
...
describe("provider adapters", () => {
  ...
  it("detects widenet not-found", async () => {
    globalThis.fetch = vi.fn(async () => new Response(JSON.stringify({ status: 404, ok: false }), { status: 200 })) as typeof fetch;
    await expect(widenetProvider.fetch("00000000", 100)).rejects.toBeInstanceOf(ProviderNotFoundSignal);
  });
  ...
  it("converts http failures into ProviderRequestError", async () => {
    ...
    await expect(brasilapiProvider.fetch("01310100", 100)).rejects.toBeInstanceOf(ProviderRequestError);
  });
});
